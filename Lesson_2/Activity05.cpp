...
std::vector<int> merge(const std::vector<std::vector<int>>& input_value)
{
	...
	{
		heap.push_back({ input_value[i][0], i, 0 });
	...
	while (!heap.empty())
	{
		...
		if (nextIndex < input_value[min.listPosition].size())
		{
			heap.push_back({ input_value[min.listPosition][nextIndex], min.listPosition, nextIndex });
			std::push_heap(heap.begin(), heap.end(), comparator);
		}
	}
	return result;
