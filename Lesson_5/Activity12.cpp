...
template<typename T>
auto Welsh_Powel_coloring(const Graph<T>& G)
{
	auto size = G.vertices();
	vector<pair<unsigned, size_t>> degrees;

	// 각 정점의 차수를 <정점ID, 차수>의 쌍으로 취합
	...
	while (true)
	{
		for (auto const i : degrees)
		{
			...
			// i번째 정점과 인접한 정점이 color_to_be_assigned 색상을 가지고 있지 않다면 i번째 정점에 color_to_be_assigned 색상을 지정
...
int main()
{
	...
	auto colors = Welsh_Powel_coloring<T>(G);
	cout << "[그래프 컬러링]" << endl;
	print_colors(colors);
}